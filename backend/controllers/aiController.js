...
const extractPromises = async (req, res) => {
  ...
  const previewPromises = await Promise.all(promises.map(async (p) => {
    let isDuplicate = false;
    let duplicateOf = null;

    if (embeddedExisting.length > 0) {
      const matches = await ragService.retrieveRelevantChunks(`${p.title} ${p.description}`, embeddedExisting, 1);
...
const compareManifestos = async (req, res) => {
  try {
    ...
    const chunks1 = await ragService.retrieveRelevantChunks(
      topic,
      m1.chunks || [],
      3
    );

    const chunks2 = await ragService.retrieveRelevantChunks(
      topic,
      m2.chunks || [],
      3
    );
